...
define(['app', 'angular', 'moment', 'cs!style', 'cs!smCss', 'smJs', 'pager', 'lazyLoad','/common/directive/header/header.js', 'cs!static/css/userCenter/wow'], function(app, angular, moment) {

  /*定义 wowCtrl 控制器*/
  app.angular.controller('userCenter/wowCtrl', ['$rootScope', '$scope', '$state', '$stateParams', '$http', function($rootScope, $scope, $state, $stateParams, $http) {
    $.extend($scope, {
      init: function() {
        this.staticScope();
        this.initAjax();
      ...
      staticScope: function() {
        app.myApp.settitle($rootScope, '我的wow-美妆商城'); // 设置 title
      ...
      initAjax: function() {
        $.pager({ // 用户消息列表
          ...
          scrollEle: '.content',
          data: {
            type: 3   // 消息类型: 0系统通知(没玩秘书) 1:用户关注 2：用户评论 3：用户送花
          },
